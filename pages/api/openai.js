...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: userProfile, error: profileError } = await supabaseAdmin
      .from('profiles')
      .select('subscription_tier, monthly_responses_used, monthly_responses_limit, business_name, business_description')
      .eq('id', user.id)
      .single();
    ...
    if (userProfile.monthly_responses_used >= userProfile.monthly_responses_limit) {
      return res.status(403).json({ 
        ...
        currentUsage: userProfile.monthly_responses_used,
        limit: userProfile.monthly_responses_limit,
    ...
    const businessName = userProfile.business_name || 'Our Company';
    const businessDescription = userProfile.business_description || 'A business committed to excellent customer service';
