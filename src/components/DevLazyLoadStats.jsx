...
const DevLazyLoadStats = () => {
  ...
  const [isDev] = useState(process.env.NODE_ENV === 'development' || process.env.NEXT_PUBLIC_DEV_MODE === 'true');
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 bg-white rounded-lg shadow-lg p-4 w-80 max-h-80 overflow-auto">
      ...
      <div className="text-xs text-gray-500 mb-2">
        Press Ctrl+Shift+L to toggle this panel
      </div>
        ...
        <tbody>
          {Object.entries(stats).map(([name, data]) => (
            <tr key={name}>
              ...
              <td className="py-1">
                <span className={`inline-block w-2 h-2 rounded-full ${data.loaded ? 'bg-green-500' : 'bg-yellow-500'} mr-1`}></span>
